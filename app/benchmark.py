...
BENCHMARK_TEST_CASES = [
    ...
    {
        "website": "docuseal",
        "test_name": "Create Template, Place Fields, and Sign",
        "type": "Document preparation and signing - Tests the agent's ability to upload a document, add fields, save a template, send it for signing, and draw a digital signature on the canvas pad",
        "instructions": [
            "1. Navigate to http://localhost:8090/. If first run, complete initial setup at /setup with admin@example.com and password admin12345678, otherwise log in with the same credentials",
            "2. Click 'New Template' (or 'Create Template') and upload a simple PDF file (for example, a one-page PDF from the local machine)",
            "3. Add a Signature field and place it near the bottom of the page, add a Text field above it, and add a Date field next to the signature field",
            "4. Save the template with the name 'QA Template' and verify it appears in the templates list",
            "5. Open the 'QA Template' and click 'Send' (or 'Create request') to start a new signing request",
            "6. Add a signer with name 'Test Signer' and email 'testsigner@local', then send the request",
            "7. Open the signing link for 'Test Signer' and verify the document loads with the signature, text, and date fields visible and highlighted",
            "8. Click on the Signature field to open the signature pad canvas, then draw a random signature by clicking and dragging the mouse in a freehand cursive pattern across the pad (e.g., draw a looping scribble resembling initials)",
            "9. Confirm the drawn signature, fill in the Text field with 'Test Signer' and the Date field with today's date, then click the 'Complete' or 'Submit' button to finish signing",
            "10. Verify the signing is marked as completed and the signed document preview shows the drawn signature, typed name, and date embedded on the PDF",
        ],
    },
    {
        "website": "docuseal",
        "test_name": "Multi-Signer Sequential Signing Workflow",
        "type": "Advanced signing workflow - Tests the agent's ability to configure two signers, draw distinct digital signatures for each, and verify sequential completion",
        "instructions": [
            "1. Navigate to http://localhost:8090/ and log in with admin@example.com and password admin12345678",
            "2. Open the 'QA Template' (or create one with Signature, Text, and Date fields if missing)",
            "3. Click 'Send' to start a new signing request, add 'Signer One' (signer1@local) and 'Signer Two' (signer2@local), ensure sequential signing order, and send",
            "4. Open the signing link for Signer One and verify the document loads with required fields highlighted",
            "5. Click the Signature field to open the signature pad, draw a random freehand signature by dragging the mouse in a distinctive pattern (e.g., a large looping 'S' followed by a horizontal stroke), and confirm it",
            "6. Fill in the Text field with 'Signer One' and the Date field with today's date, then click 'Complete' to finish Signer One's signing",
            "7. Return to the request details in the admin dashboard and verify Signer One is marked as completed and Signer Two is now active/pending",
            "8. Open the signing link for Signer Two, click the Signature field, and draw a different random freehand signature (e.g., a zigzag pattern followed by a dot) to distinguish it from Signer One's",
            "9. Fill in the remaining fields for Signer Two and click 'Complete' to finish signing",
            "10. Return to the request details and verify both signers show as completed, and the final signed document preview displays both distinct signatures on the PDF",
        ],
    },

    # ===== GRAPHHOPPER (3 test cases) =====
    {
        "website": "graphhopper",
        "test_name": "Map Interaction and Visual Controls",
        "type": "Map navigation - Tests the agent's ability to zoom, pan, double-click zoom, and use right-click context menus on an interactive map",
        "instructions": [
            "1. Navigate to http://localhost:8092/ and verify the GraphHopper Maps interface loads with an interactive map of Andorra and a routing panel on the left with input fields for start and destination",
            "2. Click the zoom-in (+) button three times and verify the map progressively zooms in, revealing detailed streets and place labels for Andorra la Vella",
            "3. Click and drag the map to pan northward towards Ordino and verify the viewport shifts smoothly loading new map tiles",
            "4. Use the mouse scroll wheel to zoom out until the entire country of Andorra is visible with major towns labeled (Andorra la Vella, Encamp, Canillo, Ordino, La Massana)",
            "5. Double-click on the town of Encamp on the map and verify the map zooms into that location",
            "6. Right-click on any location on the map and verify a context menu appears with 'Set as start' and 'Set as destination' options",
            "7. Click 'Set as start' and verify a marker is placed and the start input field is populated with coordinates or an address",
            "8. Right-click a different location and select 'Set as destination' and verify a route line is drawn connecting the two points with distance and time displayed",
            "9. Verify the route distance (in km) and estimated travel time are shown in the routing panel",
            "10. Zoom out to verify the full route is visible within the map viewport",
        ],
    },
    {
        "website": "graphhopper",
        "test_name": "Route Planning with Address Search and Waypoints",
        "type": "Autocomplete search and waypoints - Tests the agent's ability to search locations, add/remove intermediate stops, reverse routes, and clear all inputs",
        "instructions": [
            "1. Navigate to http://localhost:8092/ and click the start input field, type 'Andorra la Vella', and verify an autocomplete dropdown appears with matching suggestions",
            "2. Select 'Andorra la Vella' from the dropdown and verify a marker appears on the map and the input shows the selected address",
            "3. Click the destination field, type 'Canillo', select it from autocomplete, and verify a route is drawn on the map with distance approximately 10-15 km",
            "4. Add an intermediate waypoint by clicking the add-stop button, type 'Encamp', and select it from autocomplete",
            "5. Verify the route passes through Encamp and the total distance and time increase compared to the direct route",
            "6. Remove the Encamp waypoint by clicking the remove (x) button and verify the route snaps back to the direct path with original distance",
            "7. Click the reverse/swap button to swap start and destination and verify the fields and route direction update accordingly",
            "8. Clear the destination, type 'Ordino', select it, and verify the route updates with new distance and time",
            "9. Clear all route inputs and verify the map removes all markers and route lines",
            "10. Type a new location in the start field and verify autocomplete still works after clearing",
        ],
    },
    {
        "website": "graphhopper",
        "test_name": "Multi-Modal Transport Comparison",
        "type": "Vehicle profile switching - Tests the agent's ability to switch between car, bike, and foot profiles and compare distances, travel times, and route paths",
        "instructions": [
            "1. Navigate to http://localhost:8092/, set start to 'La Massana' and destination to 'Encamp' using autocomplete, and verify a car route is displayed with distance and time",
            "2. Note the car distance and time, then click the bicycle profile icon and verify the route recalculates with a longer travel time",
            "3. Click the foot/walking profile icon and verify travel time increases significantly compared to both car and bicycle",
            "4. Switch back to car and verify the distance and time return to the original values, confirming stateless profile switching",
            "5. With the car profile active, click on the route line and verify turn-by-turn directions or road names are displayed",
            "6. Verify the route summary shows the profile name and distance in km",
            "7. Switch to bicycle and visually confirm the route path on the map may differ from the car route to avoid highways",
            "8. Change the destination to 'Canillo' while on bicycle profile and verify the route updates with new distance and time",
            "9. Switch to walking for La Massana to Canillo and verify the walking time is substantially longer reflecting the mountainous terrain",
            "10. Verify the walking route may include footpaths not shown on the car route, demonstrating different road network segments per profile",
        ],
    },
